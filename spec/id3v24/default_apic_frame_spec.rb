...
describe ID3V24::APICFrame, "when creating a new APIC (picture) frame with defaults" do
  ...
  it "should default to having a blank description" do
    expect(@saved_frame.description).to eq("cover image")
  end
  
  it "should default to having an image type of 'image/jpeg'" do
    expect(@saved_frame.mime_type).to eq('image/jpeg')
  end
  
  it "should default to a picture type of 3 ('Cover (front)')" do
    expect(@saved_frame.picture_type).to eq("\x03")
  end
  
  it "should default to a picture type name of 'Cover (front)'" do
    expect(@saved_frame.picture_type_name).to eq("Cover (front)")
  end
  
  it "should safely retrieve the picture data" do
    expect(Digest::SHA1.hexdigest(@saved_frame.value)).to eq(Digest::SHA1.hexdigest(@random_data))
  end
  
  it "should have a consistent pretty description with default values set" do
    expect(@saved_frame.to_s_pretty).to eq("Attached Picture (cover image) of image type image/jpeg and class Cover (front) of size #{Mp3InfoHelper::TEST_PRIME}")
  end
end
