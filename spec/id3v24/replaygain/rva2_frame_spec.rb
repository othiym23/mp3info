...
describe ID3V24::RVA2Frame, "when strictly interpreting the ID3v2.4 specification" do
  ...
  it "should return a parse error if there are some extra bytes in the raw data" do
    lambda { ID3V24::RVA2Frame.from_s("test\x00\x01\x00\x00\x00\x00") }.should raise_error(ID3V24::RVA2ParseError)
    lambda { ID3V24::RVA2Frame.from_s("test\x00\x01\x00\x00\x00\x00\x00") }.should raise_error(ID3V24::RVA2ParseError)
    lambda { ID3V24::RVA2Frame.from_s("test\x00\x01\x00\x00\x00\x00\x00\x00") }.should raise_error(ID3V24::RVA2ParseError)
  end
  
  it "should find 3 adjustments in a raw dump of an example frame, with their values" do
    frame = ID3V24::RVA2Frame.from_s("test\x00\x08\x04\x00\x00\x06\xfc\x00\x10\x80\x00\x07\x02\x00\x08\x80")
  ...
  it "should correctly handle peak gain bit widths that are not modulo 8 bits wide (e.g. 5 bits)" do
    frame = ID3V24::RVA2Frame.from_s("test\x00\x01\x00\x00\x05\x00")
    frame.adjustments.first.peak_gain_bit_width.should == 5
    frame.adjustments.first.peak_gain.should == 0
    
    frame = ID3V24::RVA2Frame.from_s("test\x00\x01\x00\x00\x05\x55")
    frame.adjustments.first.peak_gain_bit_width.should == 5
    frame.adjustments.first.peak_gain.should == 21
    
    frame = ID3V24::RVA2Frame.from_s("test\x00\x01\x00\x00\x05\xaa")
    frame.adjustments.first.peak_gain_bit_width.should == 5
    frame.adjustments.first.peak_gain.should == 10
    
    frame = ID3V24::RVA2Frame.from_s("test\x00\x01\x00\x00\x05\xff")
    frame.adjustments.first.peak_gain_bit_width.should == 5
  ...
  it "should correctly handle peak gain bit widths that are not modulo 8 bits wide (e.g. 22 bits)" do
    frame = ID3V24::RVA2Frame.from_s("test\x00\x01\x00\x00\x16\x00\x00\x00")
    frame.adjustments.first.peak_gain_bit_width.should == 22
    frame.adjustments.first.peak_gain.should == 0
    
    frame = ID3V24::RVA2Frame.from_s("test\x00\x01\x00\x00\x16\x55\x55\x55")
    frame.adjustments.first.peak_gain_bit_width.should == 22
    frame.adjustments.first.peak_gain.should == 1_398_101
    
    frame = ID3V24::RVA2Frame.from_s("test\x00\x01\x00\x00\x16\xaa\xaa\xaa")
    frame.adjustments.first.peak_gain_bit_width.should == 22
    frame.adjustments.first.peak_gain.should == 2_796_202
    
    frame = ID3V24::RVA2Frame.from_s("test\x00\x01\x00\x00\x16\xff\xff\xff")
    frame.adjustments.first.peak_gain_bit_width.should == 22
...
describe ID3V24::RVA2Frame, "when parsing a simple RVA2 (ID3v2.4 replaygain) frame containing one adjustment of -2dB" do
  ...
  before :all do
    ...
    tag = { "RVA2" => ID3V24::Frame.create_frame_from_string("RVA2", @rva2) }
