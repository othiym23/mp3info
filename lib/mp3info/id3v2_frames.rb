...
module ID3V24
  ...
  class Frame
    ...
    def self.create_frame(type, value)
      ...
      warn("ID3V24::Frame.create_frame(type='#{type}',value=[#{value.inspect}]) => klass=[#{klass}]") if $DEBUG

      if klass
        warn("...klass='#{klass}'") if $DEBUG
        klass.default(value)
      else
    ...
    def self.create_frame_from_string(type, value)
      klass = find_class(type)
      warn("ID3V24::Frame.create_frame_from_string(type='#{type}',value=[#{value[0..255].inspect}...]) =>...") if $DEBUG
      ...
      if klass
        ...
        case type[0, 1]
        when "T"
          warn("...klass='ID3V24::TextFrame'") if $DEBUG
          TextFrame.from_s(value, type)
        when "W"
          warn("...klass='ID3V24::LinkFrame'") if $DEBUG
          LinkFrame.from_s(value, type)
        else
          warn("...klass='ID3V24::Frame'") if $DEBUG
    ...
    def initialize(type, value)
      @type = type
      @value = value
      warn("...frame value is [#{(value.inspect.size > 255) ? "#{value.inspect[0..255]}...\"" : value.inspect}]") if $DEBUG
    ...
    def self.find_class(type)
      ID3V24.const_get(:"#{type.tr("\000", "")}Frame")
  ...
  class TextFrame < Frame
    ...
    def self.split_encoded(encoding, string)
      ...
      warn("ID3V24::TextFrame.split_encoded(encoding=#{encoding},string=[#{string[0..255].inspect}...]) => [prefix='#{prefix.inspect}',remainder=[#{remainder ? remainder[0..255].inspect : "nil"}...]]") if $DEBUG
      [prefix, remainder]
    end
  end
  ...
  class TXXXFrame < TextFrame
    ...
    def ==(other)
      other.respond_to?(:value) && @value == other.value &&
        other.respond_to?(:encoding) && @encoding == other.encoding &&
        other.respond_to?(:description) && @description == other.description
    end
  ...
  class WXXXFrame < TextFrame
    ...
    def self.from_s(value)
      warn("raw value of WXXX frame is #{value.inspect}") if $DEBUG
      encoding, str = value.unpack("ca*")
      warn("encoding #{encoding} str #{str.inspect}") if $DEBUG
      descr, entry = split_descr(encoding, str)
  ...
  class APICFrame < TXXXFrame
    ...
    def self.from_s(value)
      warn("APICFrame.from_s(value.size=#{value.size})") if $DEBUG
      encoding, str = value.unpack("ca*")
    ...
    def ==(other)
      other.respond_to?(:value) && @value == other.value &&
        other.respond_to?(:encoding) && @encoding == other.encoding &&
        other.respond_to?(:mime_type) && @mime_type == other.mime_type &&
        other.respond_to?(:picture_type) && @picture_type == other.picture_type &&
        other.respond_to?(:description) && @description == other.description
    end
  ...
  class COMMFrame < TXXXFrame
    ...
    def self.from_s(value)
      ...
      warn("ID3V24::COMMFrame.from_s(value=[#{value.inspect}]) => [encoding=#{encoding}, lang=[#{lang}], descr=[#{descr}], entry=[#{entry}]]") if $DEBUG
      COMMFrame.new(encoding, lang, descr, entry)
    end

    def to_s
      warn("COMMFrame.to_s => [#{encoding}|#{@language || "XXX"}|#{encode_value(@encoding, @description || "").inspect}|#{encode_value(@encoding, @value).inspect}]") if $DEBUG
    ...
    def to_s_pretty
      ...
      warn("COMMFrame.to_s_pretty => [#{prefix}|#{@value}]") if $DEBUG
      "#{"#{prefix}: " if prefix && prefix != ""}#{@value}"
    end

    def ==(other)
      other.respond_to?(:value) && @value == other.value &&
        other.respond_to?(:encoding) && @encoding == other.encoding &&
        other.respond_to?(:language) && @language == other.language &&
        other.respond_to?(:description) && @description == other.description
    end
  end
  ...
  class PRIVFrame < Frame
    ...
    def ==(other)
      other.respond_to?(:value) && @value == other.value &&
        other.respond_to?(:owner) && @owner == other.owner
    end
  end
  ...
  class TCMPFrame < TextFrame
    ...
    def self.from_s(value)
      ...
      TCMPFrame.new(encoding, string == "1")
    end
    ...
    def to_s_pretty
      "This track is #{"not " unless value}part of a compilation."
    end
  end
  ...
  class UFIDFrame < Frame
    ...
    def ==(other)
      other.respond_to?(:value) && @value == other.value &&
        other.respond_to?(:namespace) && @namespace == other.namespace
    end
  end
  ...
  class RVA2Frame < Frame
    ...
    def self.default(value)
      ...
      new("track", [default_adjustment])
    end

    def self.from_s(value)
      id, raw_adjustment_list = value.split("\x00", 2)
      warn("#{name}.from_s(value=#{value.inspect}) => id=[#{id.inspect}] raw_adjustment_list=[#{raw_adjustment_list.inspect}]") if $DEBUG
      new(id, parse_adjustments(raw_adjustment_list))
    end
    ...
    def self.parse_adjustments(raw_value)
      ...
      cur_pos = 0
      warn("RVA2Frame.parse_adjustments(raw_value=#{raw_value.inspect}) => total_bytes=[#{total_bytes}]") if $DEBUG

      while total_bytes - cur_pos > 0
        ...
        cur_pos += 1
        warn("RVA2Frame.parse_adjustments channel_code=[#{channel_code.inspect}] cur_pos=[#{cur_pos}]") if $DEBUG
        ...
        cur_pos += 2
        warn("RVA2Frame.parse_adjustments adjustment=[#{adjustment.inspect}] cur_pos=[#{cur_pos}]") if $DEBUG

        # figure out how many bits' worth of peak gain scale adjustment there is
        raise(RVA2ParseError, "insufficient bytes left to parse out another adjustment") if cur_pos + 1 > total_bytes
        peak_gain_bit_size = raw_value[cur_pos].ord
        cur_pos += 1
        warn("RVA2Frame.parse_adjustments peak_gain_bit_size=[#{peak_gain_bit_size.inspect}] cur_pos=[#{cur_pos}]") if $DEBUG
        ...
        if peak_gain_bit_size > 0
          ...
          peak_gain_value = peak_gain_bytes.to_binary_array[-peak_gain_bit_size..].to_binary_decimal
          cur_pos += peak_gain_byte_width
          warn("RVA2Frame.parse_adjustments peak_gain_bytes=[#{peak_gain_bytes.inspect}] peak_gain_value=[#{peak_gain_value}] cur_pos=[#{cur_pos}]") if $DEBUG
        end
  ...
  # Base class for RVA (ID3v2.2) and RVAD (ID3v2.3) volume adjustment frames.
  # Extracts shared logic for bit-field manipulation, dB conversion, and
  # channel gain/peak accessors. Subclasses must define:
  #   - right_channel  (constant for the "right" channel)
  #   - left_channel   (constant for the "left" channel)
  #   - channel_to_offset(channel)
  #   - peak_offset(channel)
  class BaseVolumeAdjustmentFrame < Frame
    def bit_width
      @value[1].ord
    end
    ...
    def right_gain
      get_db(right_channel)
    end

    def right_gain=(value)
      set_db(right_channel, value)
    end
    ...
    def right_peak
      get_peak(right_channel)
    end

    def right_peak=(value)
      set_peak(right_channel, value)
    end

    def left_peak
      get_peak(left_channel)
    end

    def left_peak=(value)
      set_peak(left_channel, value)
    end
    ...
    def self.default_raw_string(right_const, left_const)
      string = ""
      # 1 byte indicating that the right and left channels are adjusted positively
      string << (right_const | left_const).chr
      # 1 byte indicating the width of the field in which we're storing adjustments
      string << 16.chr
      # 0.0 dB adjustment for both channels by default
      string << "\x00\x00\x00\x00"
      # peak values of 0 for both default channels (better safe than sorry)
      string << "\x00\x00\x00\x00"
  ...
  class RVAFrame < BaseVolumeAdjustmentFrame
    RIGHT = 0x01
    LEFT = 0x02
  ...
  class RVADFrame < BaseVolumeAdjustmentFrame
    FRONT_RIGHT = 0x01
    FRONT_LEFT = 0x02
    REAR_RIGHT = 0x04
    REAR_LEFT = 0x08
  ...
  class RGADAdjustment
    ...
    def valid?
      (type_code == TYPE_RADIO ||
       type_code == TYPE_AUDIOPHILE) &&
        origin_code != ORIGIN_UNSPECIFIED
    end
    ...
    def raw_adjustment=(value)
      warn("ID3V24::RGADFrame.raw_adjustment= value #{value} raw_string #{@raw_string.inspect}") if $DEBUG
      new_value = @raw_string.to_binary_array
      new_value[6] = ((value < 0) ? 1 : 0)
      new_value[7, 9] = [value.abs, 2**10 - 1].min.to_binary_array(9)
      warn("ID3V24::RGADFrame.raw_adjustment= new_value #{new_value.to_binary_string.inspect}") if $DEBUG
  ...
  class RGADFrame < Frame
    ...
    def track_gain=(value)
      ...
      elsif value.is_a?(RGADAdjustment)
