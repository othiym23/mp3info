...
class MPEGHeader
  ...
  def summary
    "[ #{bitrate}kbps @ #{sample_rate / 1000.0}kHz - #{mode}#{" +error" if error_protected?} ]"
  end
  ...
  def sample_rate
    # Use raw @version code (0=2.5, 2=2.0, 3=1.0) to avoid float comparison
    sample_rate = case @version
    when 0
      SAMPLE_RATES[@sample_rate_code][2]
    when 2
      SAMPLE_RATES[@sample_rate_code][1]
    when 3
      SAMPLE_RATES[@sample_rate_code][0]
    else
  ...
  def description
    <<~DONE
      MPEG header information:
        ...
        Frame header is #{"not " unless valid?}valid.
        ...
        Audio stream is #{"not " unless error_protected?}error-protected.
        Audio stream is #{"not " unless private_stream?}private.
        Audio stream is #{"not " unless original_stream?}original.
        Audio stream is #{"not " unless padded_stream?}padded.
        Audio stream is #{"not " unless copyrighted_stream?}copyrighted.
  ...
  def lsf
    # Use raw @version code (3=1.0) to avoid float comparison
    case @version
    when 3
      0
    else
