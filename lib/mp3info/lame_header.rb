...
class LAMEReplayGainType
  ...
  def adjustment
    if sign
      -1 * (@raw_frame.to_binary_array[7..].to_binary_decimal / 10.0)
    else
      @raw_frame.to_binary_array[7..].to_binary_decimal / 10.0
    end
  end
...
class LAMEHeader
  ...
  def bitrate
    ...
    if (8..321).cover?(preset_number) && bitrate_number >= 255
      preset_number
    else
      bitrate_number
    end
  ...
  def description
    <<~OUT
      LAME tag:
        ...
        LAME tag is #{"not " unless valid?}valid.
        ...
        Lowpass filter   : #{lowpass_filter / 1000} kHz#{replay_gain}
